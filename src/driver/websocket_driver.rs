...
pub const WEBSOCKET_DRIVER: &str = "udp_driver";
...
impl Driver for WebSocketDriver {
    async fn send(&self, packet: &MavFramePacket) {
        let mut lock = self.write_half.clone();
        ...
        let message = Message::text(packet.to_string()); // This is a placeholder. Customize as needed.
        let _ = lock
            .send(packet)
            .await
        ...
        if let Some(message_result) = lock.next().await {
            match message_result {
                Ok(message) => match message {
                    Message::Text(text) => {
                        Some(MavFramePacket::from_text(&text)) // This is a placeholder. Customize as needed.
                    }
